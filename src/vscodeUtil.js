...
class FileAction {
   ...
   */
  getDirPath(context, templatePath) {
   ...
   * @returns
   */
  getExtensionFileVscodeResource(context, templatePath, filepath) {
    const resourcePath = path.join(context.extensionPath, templatePath);
   ...
   */
  mhCreateFile(context, templatePath, filedata, callback) {
   ...
   * @param {*} templatePath 相对于插件根目录的html文件相对路径
   */
  mhRemoveFile(context, templatePath, callback) {
   ...
   */
  mhUpdateFile(context, templatePath, upData, callback) {
    this.mhReadFile(context, templatePath, (err, oldData) => {
   ...
   */
  mhInsertData(context, templatePath, insData, type = 'data', callback) {
    this.mhReadFile(context, templatePath, (err, oldData) => {
      if (err) {
        callback(err);
      } else {
   ...
   * @param {*} id 需要删除的数据的id
   */
  mhRemoveData(context, templatePath, id, callback) {
    this.mhReadFile(context, templatePath, (err, oldData) => {
      if (err) {
        callback(err, null);
      } else {
   ...
   *
   */
  mhClearData(context, templatePath, callback) {
    this.mhReadFile(context, templatePath, (err, oldData) => {
      if (err) {
        callback(err, null);
      } else {
